...
export function deriveDashboardSummary({ planning = {}, productionActuals = [], productionMrp = {}, materialReconciliation = { summary: {}, rows: [] }, selectedDay = null, reconciliationAvailable = false, productionLine = "blowMoulding" }) {
  ...
  const originalPlannedSets = productionPlans.reduce((sum, entry) => sum + Math.max(0, getOriginalPlanQty(entry)), 0);
  const plannedSets = productionPlans.reduce((sum, entry) => sum + Math.max(0, getEffectivePlanQty(entry)), 0);
